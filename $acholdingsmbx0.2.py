...
# -*- coding: utf-8 -*-
"""
AC'S SMBX 1-tech — All-in-One Edition
...
class Layer:
    ...
    def remove_tile(self, tile):
        key = (tile.rect.x, tile.rect.y)
        if self.tiles.has(tile): self.tiles.remove(tile)
...
class Editor:
    ...
    def place_object(self, gx, gy):
        ...
        if cat == "NPCs":
            ...
            layer.npcs.add(npc)
            self.push_undo({'undo': lambda l=layer, n=npc: l.npcs.remove(n) if l.npcs.has(n) else None,
                            'redo': lambda l=layer, n=npc: l.npcs.add(n)})
        elif cat == "BGOs":
            bgo = BGO(gx, gy, self.sidebar.selected_item, layer_idx=li)
            layer.bgos.add(bgo)
            self.push_undo({'undo': lambda l=layer, b=bgo: l.bgos.remove(b) if l.bgos.has(b) else None,
        ...
        for group in (layer.npcs, layer.bgos):
            for obj in list(group):
                if obj.rect.x == gx and obj.rect.y == gy:
                    if group.has(obj): group.remove(obj)
                    self.push_undo({'undo': lambda g=group, o=obj: g.add(o),
                                    'redo': lambda g=group, o=obj: g.remove(o) if g.has(o) else None})
                    return
    ...
    def paste_clipboard(self):
        ...
        def _undo_paste(items=pasted):
            for kind, lyr, obj in items:
                ...
                elif kind=='bgo': lyr.bgos.remove(obj) if lyr.bgos.has(obj) else None
                elif kind=='npc': lyr.npcs.remove(obj) if lyr.npcs.has(obj) else None
    ...
    def _delete_object(self, obj):
        ...
        elif isinstance(obj, BGO): layer.bgos.remove(obj) if layer.bgos.has(obj) else None
        elif isinstance(obj, NPC): layer.npcs.remove(obj) if layer.npcs.has(obj) else None
...
def main_menu(screen):
    ...
    while True:
        ...
        draw_text(screen,"AC'S SMBX 1-tech",
...
def main():
    ...
    pygame.init()
    pygame.display.set_caption("AC'S SMBX 1-tech")
